...
/*
	...
	3. Only quicksorts TOTAL_ELEMS / QSORT_MAX_THRESH partitions, leaving
	   insertion sort to order the QSORT_MAX_THRESH items within each partition.
...
template size_t vanilla_partition<float>(float input[], size_t low, size_t high);

// Recursive calls
template size_t vanilla_quicksort<int>(int input[], int low, int high);
